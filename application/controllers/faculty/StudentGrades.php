...
class StudentGrades extends CI_Controller {
    ...
    public function index() {
        ...
        if($this->session->logged_in && $this->session->access == 1) {
            ...
            if(!file_exists(APPPATH.'views/faculty/student_grades/'.$page.'.php')) {
    ...
    public function print() {
        ...
        if($this->session->logged_in && $this->session->access == 1) {
            ...
            $data['data'] = $this->Student_Grade_model->show_students($id, $slug_subject);
            $html = $this->load->view('faculty/student_grades/print', $data, TRUE);
            ...
            $dompdf->render();
            $dompdf->stream('student-grades.pdf', array("Attachment" => FALSE));
            $dompdf->exit();
